...
set -e
[[ "$hub_status" == 1 ]]
if [[ "$(yq eval -r '.last_overall_status' "${TEST_DIRECTORY}/hub-state/federation-hub-state.json")" != degraded ]]; then
    printf 'Expected degraded hub state; hub log follows:\n' >&2
    /usr/bin/tail -n 30 "${TEST_DIRECTORY}/hub.log" >&2
    for report in "${TEST_DIRECTORY}/incoming/"*.json; do
        [[ -e "$report" ]] || continue
        printf 'Report %s: node=%s timestamp=%s type=%s\n' "$report" \
            "$(yq eval -r '.node_id // ""' "$report")" \
            "$(yq eval -r '.timestamp // ""' "$report")" \
            "$(yq eval -r '.services | type' "$report")" >&2
    done
    exit 1
fi
